...
cambio_19_vs_previos <- 
  ...
  summarise(num = n()) %>% 
  ungroup() %>%  
  mutate(cambio_vs_2017 = round(((num - lag(num, n = 1))/lag(num, n = 1))*100, 1), 
         cambio_vs_2016 = round(((num - lag(num, n = 2))/lag(num, n = 2))*100, 1),
...
cambio_2019_vs_2017 <- 
  ...
  group_by(ano) %>% 
  summarise(num = n()) %>% 
  mutate(dias_transcurridos = as.numeric(as_date("2019-02-28") - as_date("2019-01-01")),
         proyeccion = ifelse(ano == 2017, num, (num/dias_transcurridos)*365), 
         cambio_19_17 = round((proyeccion - lag(proyeccion))/lag(proyeccion)*100, 1),
         proyeccion = comma(proyeccion), 
         sentido = ifelse(cambio_19_17 > 0, "mayor", "menor"),
         cambio_19_17 = abs(cambio_19_17))
```


En los primeros tres meses de 2019 se han acumulado **`r datos_delito %>% filter(ano == 2019) %>% summarise(num = comma(n()))`** carpetas de investigación por este delito en la CDMX. Esta cifra es **`r cambio_19_vs_previos %>% filter(ano == 2019) %>% select(cambio_vs_2017)`%** `r cambio_19_vs_previos %>% filter(ano == 2019) %>% select(sentido_19_vs_17)` que el número de carpetas acumulado hasta marzo de 2017 (`r cambio_19_vs_previos %>% filter(ano == 2017) %>% select(num)`) y **`r cambio_19_vs_previos %>% filter(ano == 2019) %>% select(cambio_vs_2016)`%** `r cambio_19_vs_previos %>% filter(ano == 2019) %>% select(sentido_19_vs_16)` que en el mismo período durante 2016 (`r cambio_19_vs_previos %>% filter(ano == 2016) %>% select(num)`). La base de datos publicada por la ADIP no incluye carpetas de investigación para esta categoría de delito en 2018.
...
incremento_tasa_trimestral <- 
  tasa_trimestral_anualizada %>% 
  mutate(cambio_por_tasa_19_17 = round(((tasa_x_100k - lag(tasa_x_100k, n = 1))/lag(tasa_x_100k, n = 1))*100, 1),
         cambio_por_tasa_19_16 = round(((tasa_x_100k - lag(tasa_x_100k, n = 2))/lag(tasa_x_100k, n = 2))*100, 1),
         sentido_19_17 = (ifelse(cambio_por_tasa_19_17 > 0, "mayor que", "menor que")),
         sentido_19_16 = (ifelse(cambio_por_tasa_19_16 > 0, "más grande que", "menor que")),
...
Estas cifras revelan que la tasa trimestral anualizada de 2019 es `r incremento_tasa_trimestral %>% filter(ano == 2019) %>% select(cambio_por_tasa_19_17)` la tasa para el mismo período en 2017 y `r incremento_tasa_trimestral %>% filter(ano == 2019) %>% select(cambio_por_tasa_19_16)` en 2016.
...
cambio_num_acumulado_delitos_alcaldia %>% 
  ...
  scale_x_continuous(breaks = c(2017, 2019), labels = c("Ene. - mar. \'17\n", "Ene. - mar. \'19\n"), 
                     limits = c(2017, 2019.05)) +
  scale_color_manual(values = c("tomato", "#66bd63")) +
  labs(title = str_wrap(str_to_upper(str_c("número acumulado de carpetas de investigación por ", unique(datos_delito$categoria_de_delito), ", primer trimestre de 2019 vs. primer trimestre de 2017, por alcaldía", sep = "")), width = 57),
...
Como muestra la gráfica que se presenta a continuación, las tres alcaldías con el mayor incremento en el número de carpetas de investigación por `r datos_delito %>% summarise(delito = str_to_lower(last(categoria_de_delito)))` en el arranque de 2019 vs. el mismo período en 2017 son `r stri_replace_last(str_c(top_3_cambio$alcaldia, collapse = ", "), replacement = " e", regex = ",")  `. 
...
### Tasa trimestral anualizada por alcaldía para 2017 y 2019
tasa_trimestral_anualizada_alcaldia <- 
  ...
  ungroup() %>%
  complete(alcaldia, nesting(ano)) %>% # Completar renglones que faltan en algunas alcadías porque no hubo carpeta en el año correspondiente
  mutate(num_ci = ifelse(is.na(num_ci), # Reemplazar NAs de num_ci por 0
                              0, num_ci),
         etiqueta_alcaldia = ifelse(ano == 2019, alcaldia, "")) %>%  # Generar etiquetas para gráfica
...
tasa_trimestral_anualizada_alcaldia %>% 
  ...
  scale_x_continuous(breaks = c(2017, 2019), labels = c("Ene. - mar. \'17\n", "Ene. - mar. \'19\n"), 
                     limits = c(2017, 2019.05)) +
  scale_color_manual(values = c("tomato", "#66bd63")) +
  labs(title = str_wrap(str_to_upper(str_c("tasa trimestral anualizada de carpetas de investigación por ", unique(datos_delito$categoria_de_delito), ", primer trimestre de 2019 vs. primer trimestre de 2017, por alcaldía", sep = "")), width = 57),
       subtitle = "Tasa por cada 100 mil habitantes",
       x = NULL,
